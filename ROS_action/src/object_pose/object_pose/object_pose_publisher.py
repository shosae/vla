...
class ObjectPosePublisher(Node):
    ...
    def convert_to_robot_angle(self, yolo_angle_deg):
        ...
        return (-yolo_angle_deg) % 360  # 각도 반전 후 0~360 범위 보정
    ...
    def callback(self, msg: Image):
        ...
        cups['area'] = (cups.xmax - cups.xmin) * (cups.ymax - cups.ymin)
        cup = cups.sort_values('area', ascending=False).iloc[0]

        cx = (cup.xmin + cup.xmax) / 2.0
        bbox_h = cup.ymax - cup.ymin
        ...
        # robot_angle_rad = math.radians(robot_angle_deg) # 필요시 사용
        
        # 거리 계산
        focal = (h/2) / np.tan(np.deg2rad(self.h_fov/2))
        distance = (0.15 * focal) / bbox_h  # 컵 실제 높이 15cm 가정

        # 메시지 생성
        object_info = ObjectInfo()
        object_info.object_id = 'cup'
        object_info.distance = float(distance)
        object_info.angle = float(robot_angle_deg)
        # object_info.angle = float(robot_angle_rad)
        
        self.pub.publish(object_info)
        self.get_logger().info(
            f'Detected cup: 이동각={robot_angle_deg:.1f}°, 거리={distance:.2f}m'
        )
